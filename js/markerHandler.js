...
AFRAME.registerComponent("markerhandler", {
  ...
  askTableNumber: function() {
    ...
    swal({
      icon:iconUrl,
      title:"welcome to our restraunt",
      content: {
        element : "input",
        attributes:{
  ...
  handleMarkerFound: function(dishes, markerId) {
    ...
    if (dish.unavailable_days.includes(days[todaysDay])) {
      ...
      model.setAttribute("visibile", true);
  ...
  handleOrder: function(tNumber, dish) {
    firebase.firestore().collection("tables").get().then((doc)=>{
      var details = doc.data();
      if(details["current_orders"][dish.id]){
        details["current_orders"][dish.id]["qunatity"] +=1;
        var currentQuantity = details["current_orders"][dish.id]["qunatity"];
